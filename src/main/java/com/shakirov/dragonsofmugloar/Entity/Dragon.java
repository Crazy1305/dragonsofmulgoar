 ...
 * Class Dragon
 * Default constructor creates a dragon that can eat a knight
 * weather considered
 */
public class Dragon {
    ...
    private class InnerDragon {    
        ...
        public void create(Knight knight) throws IllegalArgumentException, IllegalAccessException {
            ...
            if (getFieldValue(fields.get(0)) == 0) {
                ...
                if ((getFieldValue(fields.get(2))> 1)&&(points < maxPoints)) {
                    addFieldValue(fields.get(2), -1);
                    points++;
                }
                if ((getFieldValue(fields.get(1))> 1)&&(points < maxPoints)) {
                    addFieldValue(fields.get(1), -1);
                    points++;
                }
                if ((getFieldValue(fields.get(0))> 1)&&(points < maxPoints)) {
                    addFieldValue(fields.get(0), -1);
                    points++;
                }              
